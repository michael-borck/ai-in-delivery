#!/usr/bin/env bash
# Build the AI in Delivery companion site (Quarto website + slide deck) into docs/.
...
# conditional content; PDF/PPTX keep the PNG). Only images referenced in the
# deck are converted. Needs `cwebp`; skips silently if absent.
if command -v cwebp >/dev/null 2>&1; then
  for name in $(grep -oE 'images/[^)]+\.(png|webp)' content/slide-deck.md \
    ...
    if [ ! -f "$webp" ] || [ "$png" -nt "$webp" ]; then
      case "$name" in *[Qq][Rr]*) cwebp -lossless "$png" -o "$webp" >/dev/null 2>&1 || true ;;
        *) cwebp -q 85 "$png" -o "$webp" >/dev/null 2>&1 || true ;; esac
    fi
  done
fi
