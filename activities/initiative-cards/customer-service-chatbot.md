...
description: "Deflect routine queries and cut 26-hour response times — but it's customer-facing, so confident-wrong answers are a live risk. ($450K, funded.)"
---
...
The board has approved **$450K** to deploy an AI customer-service chatbot across RetailFlow's online channels. The case that won funding: customer satisfaction has fallen 78% → 68% and email response times sit at **26 hours**. The chatbot is meant to deflect routine queries (order status, returns, sizing) and cut response times dramatically.
...
Conversation/transcript data is **good** — the best of the four initiatives. Priya's honest read: *"good data foundation, ~6-month implementation, proceed with confidence."* But "good foundation" ≠ "knows your policies." Grounding the bot in current, correct policy documents (RAG) is where the real work is.
